...
# ════════════════════════════════════════════════════════════════════════════════
# 🔹 ఈ function `normalize_vector` ఏమి చేస్తుందో step-by-step గా:
# ════════════════════════════════════════════════════════════════════════════════
# 👉 ఈ function ఒక vector ను normalize (unit vector గా మార్చడం) చేస్తుంది
# 👉 అంటే: vector length = 1 అవుతుంది, direction మాత్రం same ఉంటుంది.
#
# 🔹 1. Input `v`: 
#    shape (batch_size, vector_dimension). Ex: v = [[3, 4], [1, 2]]
#
# 🔹 2. Magnitude (length) calculate చేయడం: 
#    v_mag = torch.sqrt(v.pow(2).sum(1))
#    [3, 4] → √(9 + 16) = 5
#
# 🔹 3. Zero division avoid చేయడం: 
#    v_mag = torch.max(v_mag, 1e-8)
#    (0 తో divide చేస్తే error రాకుండా safe division కోసం)
#
# 🔹 4. Shape match చేయడం: 
#    v_mag = v_mag.view(batch, 1).expand(batch, v.shape[1])
#    (డివిజన్ కి ముందు రెండు మ్యాట్రిక్స్ ల షేప్ ఒక్కటే ఉండేలా చేస్తుంది)
#
# 🔹 5. Normalize చేయడం: 
#    v = v / v_mag
#    [3, 4] → [3/5, 4/5] = [0.6, 0.8] (ఇప్పుడు length = 1 ✅)
#
# 🔹 చిన్న analogy 🧭:
# 👉 మీరు ఒక direction arrow ఉందని అనుకుందాం:
#    Original vector → length + direction
#    Normalized vector → only direction (distance remove చేస్తుంది)
# ════════════════════════════════════════════════════════════════════════════════
...
def compute_rotation_matrix_from_ortho6d(poses):
    ...
    # ════════════════════════════════════════════════════════════════════════════════
    # 🔹 PyTorch లో `-1` usage అర్థం చేసుకుందాం:
    # ════════════════════════════════════════════════════════════════════════════════
    # 👉 `-1` అంటే: "ఈ dimension size ను PyTorch automatically calculate చేయి" అని అర్థం.
    # 
    # 🔹 Example:
    # x.shape = (10, 3) (అంటే total elements = 30)
    # x.view(-1, 3, 1) అని ఇస్తే... PyTorch ఇలా think చేస్తుంది:
    # ? × 3 × 1 = 30 → కాబట్టి ? = 10. (Final shape: (10, 3, 1) అవుతుంది)
    #
    # 🔹 ఎందుకు `-1` వాడాలి? (Flexibility కోసం)
    # ❌ x.view(10, 3, 1) అని హార్డ్‌కోడ్ చేస్తే batch size ఏరోజైనా మారితే error వస్తుంది.
    # ✅ x.view(-1, 3, 1) అయితే ఏ batch size ఇచ్చినా దానంతటదే ఆటోమేటిక్ గా సర్దుకుంటుంది 👍
    #
    # 🔹 ఇక్కడ ఎందుకు వాడుతున్నాం?
    # కింద `torch.cat((x, y, z), 2)` ద్వారా మూడింటినీ columns గా కలిపి మ్యాట్రిక్స్ చేయాలి.
    # అందుకోసం నార్మల్ vector ని నిలువు column vector (batch, 3, 1) గా మార్చడానికే ఈ ఆపరేషన్!
    # (Ex: [1,2,3] → [[1], [2], [3]])
    # ════════════════════════════════════════════════════════════════════════════════
    x = x.view(-1,3,1)
    y = y.view(-1,3,1)
...
# ════════════════════════════════════════════════════════════════════════════════
# 🔹 Geodesic Distance (రెండు Rotation Matrices మధ్య Angle):
# ════════════════════════════════════════════════════════════════════════════════
# 👉 Input:
# m1, m2 → రెండు rotation matrices (batch, 3, 3)
# 👉 Output:
# theta → రెండు rotations మధ్య angle (radians లో).
# 👉 అంటే: “ఒక orientation నుంచి మరో orientation కి వెళ్లడానికి ఎంత rotate చేయాలి?”
#
# 🔹 Step-by-step explanation:
# 🔸 1. Relative rotation కనుక్కోవడం
# m = torch.bmm(m1, m2.transpose(1,2))
# ఇది R = R1 ⋅ R2^T. అంటే m1 (current) నుండి m2 (target) కి మధ్య గల డిఫరెన్స్ (m).
#
# 🔸 2. Trace ఉపయోగించి angle కనుక్కోవడం
# cos = (m[:,0,0] + m[:,1,1] + m[:,2,2] - 1) / 2
# ఇది ఒక famous formula: θ = cos^-1 ( (trace(R) - 1) / 2 )
# ఇక్కడ trace(R) = diagonal elements sum. దీని ద్వారానే angle (θ) వస్తుంది.
#
# 🔸 3. Clipping (numerical safety)
# cos = torch.min(cos, 1) మరియు cos = torch.max(cos, -1)
# సడన్ గా floating point errors వల్ల cos = 1.00001 అని రావచ్చు. కానీ acos() కి 
# valid range: `-1 ≤ cos ≤ 1`. కాబట్టి error రాకుండా ఆ రేంజ్ లోనే లాక్ చేస్తాం (clamp) ✅
# ════════════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════════
# 🔹 SVD లో Reflection (det = -1) Edge Case ని ఎలా సాల్వ్ చేస్తాం?
# ════════════════════════════════════════════════════════════════════════════════
# 👉 Problem: SVD ద్వారా R = U * V^T చేసినప్పుడు, అది ఎప్పుడూ రొటేషన్ మ్యాట్రిక్సే అవుతుందని 
# గ్యారెంటీ లేదు. కొన్నిసార్లు det = -1 వస్తుంది.
#
# ❌ Reflection (det = -1): 
# ఇది వస్తువుని రొటేట్ చేయదు, అద్దంలో (mirror) లాగ ఫ్లిప్ చేస్తుంది (Right → Left). 
# UAV/Robotics కంట్రోల్స్ లో ఇది అత్యంత ప్రమాదకరం!
#
# ✔️ Fix Idea (SVD Correction):
# reflection వచ్చినప్పుడు దాన్ని → rotation గా ఎలా మార్చాలి?
# వాడుతున్న ట్రిక్: R = U * diag(1, 1, det(UV^T)) * V^T
#
# 🔹 ఇది దేనికోసం? 
# ఒకవేళ det = -1 వస్తే, ఆఖరి ఆక్సిస్ లో ఒక -1 ని మల్టిప్లై (flip) చేసి, ఆ మిర్రర్ ఇమేజ్ ని 
# సర్దుబాటు చేసి తిరిగి ఒరిజినల్ రొటేషన్ (det = +1) గా కరెక్ట్ చేస్తుంది.
#
# 🔹 చిన్న Analogy 🪞:
# మీరు అద్దంలో మీ face చూసుకుంటే left ↔ right swap అవుతుంది. 
# కింద కోడ్ లో లాగా మళ్ళీ వేరొక ఆక్సిస్ ని కూడా flip చేస్తే, original orientation కి తిరిగి 
# వస్తుంది (Reflection × Reflection = Rotation).
# ════════════════════════════════════════════════════════════════════════════════
